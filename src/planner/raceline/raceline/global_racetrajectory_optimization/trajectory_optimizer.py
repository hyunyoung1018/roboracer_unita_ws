...
# LOCAL CHANGE (see LOCAL_CHANGES.md): opt_mintime_traj used to be imported on
# this line. It is now imported inside the 'mintime' branch below, because it
# pulls in casadi at module level and would make casadi a hard dependency of the
# whole raceline package even though only mincurv_iqp and shortest_path are run.
...
def trajectory_optimizer(input_path: str,
    ...
    if curv_opt_type == 'mincurv':
        ...
        # LOCAL CHANGE (see LOCAL_CHANGES.md): imported here rather than at
        # module level so that casadi is only required when mintime is used.
