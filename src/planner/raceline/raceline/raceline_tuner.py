#!/usr/bin/env python3
"""Edit a saved raceline by hand, in RViz, without rerunning the optimizer.

Ported from UNICORN's gb_optimizer/global_trajectory_tuner.py. It reads the
map folder's global_waypoints.json - the same file raceline_generator writes
and raceline_publisher serves - puts an interactive marker on every waypoint,
and writes the file back on Save. The car drives what comes out of it.

Between "the line is wrong here" and "regenerate the line" there was nothing:
raceline_generator.launch.xml is minutes of IQP and gives a NEW line, so a
corner you wanted moved 10 cm cost you every other corner you had already
tuned. This is the missing middle.

What changed from the original, and why:

  read/write_global_waypoints take a map DIRECTORY here, not a map name. The
  UNICORN version resolves stack_master/maps/<name>/ inside itself; ours is
  handed the path. Passing a name to ours would build a RELATIVE path - it
  would fail to read, and Save would quietly write a new tree under whatever
  directory the node was started from.

  write_global_waypoints takes the Float32 MESSAGE, not the float. Ours reads
  .data off it; the original had already unwrapped it.

  The helpers are raceline_tuner_helpers, which is the same module with the
  two thirds this file never calls removed - and skimage,
  trajectory_planning_helpers and savgol_filter removed with them.

This is an OFFLINE tool. It publishes /global_waypoints, which the race stack
also publishes, so running it next to time_trials or head_to_head puts two
publishers on the topic the car steers by. Do not.
"""
...
# The vendored subtree imports itself by absolute top-level name; raceline's
# __init__ already puts this package's directory on sys.path for that. Kept
# here too so the module stands up if it is ever imported directly.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from .readwrite_global_waypoints import read_global_waypoints, write_global_waypoints
from .raceline_tuner_helpers import straighten_1d, straighten_2d, Vel_Set, \
    entire_traj_translation, entire_traj_rotation, sampleCubicSplinesWithDerivative
from .raceline_tuner_popup import PopUpDisplay
...
class GlobalTrajTuner(Node):
    ...
    def __init__(self):
        ...
        self.glb_sp_wpnts_pub = self.create_publisher(
        ...
        if self.map_path:
            ...
            self.get_logger().info(f"Reading raceline from {self.map_path}")
            try:
                (
                ...
                ) = read_global_waypoints(self.map_path)

                self.get_logger().warn(f"{self.map_infos.data}")
            except FileNotFoundError:
                self.get_logger().error(
                    f"no global_waypoints.json in {self.map_path} - run raceline_generator.launch.xml for this map first")
        else:
            self.get_logger().error("map_dir is required and was not set; nothing to tune")
    ...
    def save_global_traj(self):
        ...
        self.get_logger().warn(f"Saved global_waypoints.json to {self.map_path}")
    ...
    def update_d(self):
        ...
        normal_right = np.column_stack(
            (np.cos(yaw_rad - np.pi / 2), np.sin(yaw_rad - np.pi / 2)))  # (N, 2)
        normal_left = np.column_stack(
            (np.cos(yaw_rad + np.pi / 2), np.sin(yaw_rad + np.pi / 2)))   # (N, 2)
    ...
    def update_psi_kappa(self):
        ...
        xy_int = helper_funcs_glob.src.interp_track.interp_track(
            reftrack=xy, stepsize_approx=0.1)[:, :2]
        ...
        if feedback.event_type == InteractiveMarkerFeedback.MENU_SELECT:
            if feedback.menu_entry_id <= len(self.main_menu_list):
                ...
                if selected_menu == "Anchor1":
                    ...
                    self.wpnts_data[:, 9] = straighten_1d(
                        self.anchor1, self.anchor2, self.wpnts_data[:, 9])
                    self.initialize_interactive_markers(self.wpnts_data)
                    self.get_logger().warn("Finish straightening velocity in z-axis!!")
                elif selected_menu == "Pos_Straighten":
                    self.push_back_waypoints()
                    self.wpnts_data[:, 3:5] = straighten_2d(
                        self.anchor1, self.anchor2, self.wpnts_data[:, 3:5])
                    ...
                    self.wpnts_data[:, 9] = Vel_Set(
                        self.anchor1, self.anchor2, user_input, self.wpnts_data[:, 9])
                    self.initialize_interactive_markers(self.wpnts_data)
                    ...
                    self.wpnts_data[:, 3:5] = entire_traj_translation(
                        reference, self.wpnts_data[:, 3:5])
                    self.refresh_geometry()
                    self.initialize_interactive_markers(self.wpnts_data)
                elif selected_menu == "entire_rotation":
                    self.push_back_waypoints()
                    self.wpnts_data[:, 3:5] = entire_traj_rotation(
                        self.anchor1, self.anchor2, self.wpnts_data[:, 3:5])
                    self.refresh_geometry()
                    self.initialize_interactive_markers(self.wpnts_data)
                ...
                if a == 0:
                    ...
                    updated_xy_yaw = sampleCubicSplinesWithDerivative(
                        reference, xy_yaw, c[b], "Pose", 1.0)
                    ...
                    self.wpnts_data[:, 9] = sampleCubicSplinesWithDerivative(
                        reference, self.wpnts_data[:, 9], c[b], "Vel", 1.0)
    ...
    def initialize_interactive_markers(self, waypoints_data):
        ...
        for idx, wpnt in enumerate(waypoints_data):
            ...
            orientation_x, orientation_y, orientation_z, orientation_w = 0.0, 0.0, np.sin(
                wpnt[7] / 2.0), np.cos(wpnt[7] / 2.0)
            ...
            if idx % self.sampling_step == 0:
                ...
                self.create_marker_control(
                    int_marker, InteractiveMarkerControl.MOVE_AXIS, "move_z", 1., 0., 1., 0.)
                # Per-axis arrows: X/Y like the vel z-axis arrow
                self.create_marker_control(
                    int_marker, InteractiveMarkerControl.MOVE_AXIS, "move_x", 1., 1., 0., 0.)
                self.create_marker_control(
                    int_marker, InteractiveMarkerControl.MOVE_AXIS, "move_y", 1., 0., 0., 1.)
        ...
        self.get_logger().info(
            f"Initialize {len(waypoints_data)} waypoints as interactive markers.")
